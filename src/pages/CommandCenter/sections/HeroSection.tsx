...
const HeroSection: Component = () => {
    ...
    const id = setInterval(() => {
      setCharCount((n) => {
        if (n >= HERO_TEXT.length) {
          clearInterval(id)
          // Small pause before marking done so the cursor blinks once first
  ...
  return (
    // animate-float gives the whole hero the slow up/down drift
    <div class="flex flex-col items-center justify-center h-full min-h-[60vh] select-none animate-float">
      ...
      <div class="relative text-center mb-10">
        ...
        {/* Pre-title label */}
        <p class="font-mono text-[10px] tracking-[0.4em] text-cyan-600 mb-4 uppercase">
          Operator Identity Confirmed
        </p>

        {/* Typewriter hero title */}
        <h1
          class="font-space text-6xl md:text-8xl text-white tracking-widest mb-2 min-h-[1.2em]"
          style={{ "text-shadow": "0 0 40px rgba(103,232,249,0.3), 0 0 80px rgba(103,232,249,0.1)" }}
        >
          {displayed()}
          {/* Cursor: blink while typing, stay solid for a beat, then hide */}
          <span
        ...
        {/* Subtitle + divider — fade in after typing completes */}
        <div
      ...
      {/* ── Quick-launch panel — fades in after typing + extra 200ms ── */}
      <div
      ...
      >
        {QUICK_LINKS.map((link) => (
          <button
            ...
            class="group relative overflow-hidden flex flex-col items-center px-5 py-3 rounded border border-white/[0.08] bg-white/[0.02] hover:border-cyan-500/40 hover:bg-cyan-500/[0.05] transition-all duration-300 min-w-[90px]"
          >
            {/* Shimmer sweep */}
      ...
      <p
        class="font-mono text-[9px] tracking-[0.3em] text-slate-700 mt-8 uppercase animate-pulse transition-opacity duration-700 delay-500"
